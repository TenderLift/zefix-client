...
if (!ZEFIX_USERNAME || !ZEFIX_PASSWORD) {
  console.error('Please set ZEFIX_USERNAME and ZEFIX_PASSWORD environment variables');
  process.exit(1);
}
...
async function paginationExample() {
  ...
  while (true) {
    try {
      ...
      console.log(`Fetched ${data.companies.length} companies (total: ${allCompanies.length})`);
...
async function referenceDataExample() {
  ...
  try {
    ...
    cantons.forEach((canton) => {
      const defaultLang = getDefaultLanguageForCanton(canton.code as any);
      console.log(`- ${canton.code}: ${canton.name} (default language: ${defaultLang})`);
    });
  } catch (error) {
...
async function rateLimitingExample() {
  ...
  console.log(`Average time per request: ${(elapsed / uids.length / 1000).toFixed(2)} seconds`);
}
